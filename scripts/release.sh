#!/usr/bin/env bash
#
# jpeg_exif_stripper Full Release Pipeline
...
#   4. Deploys to AUR (jpeg-exif-stripper, jpeg-exif-stripper-git, jpeg-exif-stripper-bin)
#
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Run the full jpeg_exif_stripper release pipeline."
            echo ""
            echo "Options:"
...
step_crates_publish() {
    ...
    if [ "$EXECUTE" = true ]; then
        if ! confirm "Publish jpeg_exif_stripper v${VERSION} to crates.io?"; then
...
step_github_release() {
    ...
    if [ "$EXECUTE" = true ]; then
        if ! confirm "Create GitHub release for jpeg_exif_stripper v${VERSION}?"; then
...
step_aur_deploy() {
    ...
    log_step "Deploying to AUR (jpeg-exif-stripper, jpeg-exif-stripper-git, jpeg-exif-stripper-bin)..."

    if [ "$EXECUTE" = true ]; then
        if ! confirm "Deploy to AUR (jpeg-exif-stripper, jpeg-exif-stripper-git, jpeg-exif-stripper-bin)?"; then
...
print_banner() {
    ...
    echo -e "${MAGENTA}${BOLD}║     📸 jpeg_exif_stripper Release Pipeline    ║${NC}"
    echo -e "${MAGENTA}${BOLD}╚══════════════════════════════════════════════╝${NC}"
    echo ""
}

print_plan() {
    echo -e "${BOLD}  Release plan for jpeg_exif_stripper v${VERSION}:${NC}"
    echo ""
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${BOLD}  jpeg_exif_stripper v${VERSION} is now live:${NC}"
    echo ""
    echo "    crates.io: https://crates.io/crates/jpeg_exif_stripper"
    echo "    GitHub:    https://github.com/Pommersche92/jpeg_exif_stripper/releases"
    echo "    AUR:       https://aur.archlinux.org/packages/jpeg-exif-stripper"
    echo "    AUR git:   https://aur.archlinux.org/packages/jpeg-exif-stripper-git"
    echo "    AUR bin:   https://aur.archlinux.org/packages/jpeg-exif-stripper-bin"
    echo ""
    echo -e "${BOLD}  Install commands:${NC}"
    echo "    cargo install jpeg_exif_stripper"
    echo "    yay -S jpeg-exif-stripper       # stable, built from source"
    echo "    yay -S jpeg-exif-stripper-git   # bleeding-edge, built from git"
    echo "    yay -S jpeg-exif-stripper-bin   # stable, prebuilt binary"
    echo ""
}
...
main() {
    ...
    if [ "$EXECUTE" = true ]; then
        if ! confirm "Proceed with release pipeline for jpeg_exif_stripper v${VERSION}?"; then
            echo ""
